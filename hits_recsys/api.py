...
# %% auto 0
__all__ = ['DEF_FMT', 'cli', 'PredictRequest', 'add_routes', 'init_logger', 'LoggingQueue', 'serve']
...
def cli(optype, # operation to peroform, one of 'train', 'eval' or 'pred'
    ...
    if optype=='train':
        ...
        serv.save(out)
        l.info(f"model trained and saved to {out}")
    elif not serv.model:
...
# %% ../nbs/02_api.ipynb 17
@call_parse
def serve(port=5000, # port to listen on
    ...
    if not serv.model: 
          ...
          return
    serv.location = './serv.pt'
    add_routes(app, serv)
    add_logging(app,q)
    serv.save(serv.location)
    if in_notebook(): nest_asyncio.apply()
    cfg = uvicorn.Config(app, port=port, log_config=None)
